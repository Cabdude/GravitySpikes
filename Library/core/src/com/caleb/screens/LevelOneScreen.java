...
public class LevelOneScreen implements Screen {
	...
	public LevelOneScreen(GravitySpikesRevamped game) {
		this.game = game;
		...
		levelOne = new TmxMapLoader().load("LevelOne.tmx");
		renderer = new OrthogonalTiledMapRenderer(levelOne, 1 / GravitySpikesRevamped.PPM);
		...
		new B2WorldCreator(world, levelOne);
	...
	public void handleInput(float dt) {
		...
		if (Gdx.input.isKeyPressed(Keys.RIGHT) || Gdx.input.isButtonPressed(Input.Buttons.RIGHT) && player.playerBody.getLinearVelocity().x <= 4f)
			player.playerBody.applyLinearImpulse(new Vector2(5f, 0), player.playerBody.getWorldCenter(), true);
		if (Gdx.input.isKeyPressed(Keys.LEFT) || Gdx.input.isButtonPressed(Input.Buttons.LEFT) && player.playerBody.getLinearVelocity().x >= -4f)
			player.playerBody.applyLinearImpulse(new Vector2(-5f, 0), player.playerBody.getWorldCenter(), true);
		...
		if (ExitPortal.win == GravitySpikesRevamped.EXIT_BIT) {
			...
			new PreferenceManager().addPreferences("One");
		}
	}
